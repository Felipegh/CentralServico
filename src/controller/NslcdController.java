...
public class NslcdController implements Serializable{
	...
	public void migrarUsuario() throws UnsupportedEncodingException, ParseException{
		NslcdDAO nslcdDAO = new NslcdDAOImpl();
		MensagemErroLDAP mensagem = new MensagemErroLDAP();
		try {
			nslcdDAO.migrate(nslcd, sistemaOperacional, isFlagAdmin);
			FacesContext.getCurrentInstance().addMessage("message", new FacesMessage(FacesMessage.SEVERITY_INFO, "","Atributo adicionado com sucesso"));
			
		} catch (LDAPException e) {
			String msg = mensagem.mensagem(e.getResultCode());
			nslcd.setMensagem(msg);
			FacesContext.getCurrentInstance().addMessage("message", new FacesMessage(FacesMessage.SEVERITY_ERROR, "",nslcd.getMensagem()));
		}
		//return "/site/nslcd/body/nslcd.xhtml?faces-redirect=true";
	}
