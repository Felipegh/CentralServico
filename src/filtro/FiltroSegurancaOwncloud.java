...
@WebFilter(urlPatterns="/site/owncloud/body/")
public class FiltroSegurancaOwncloud implements Filter {
	...
	public void doFilter(ServletRequest request, ServletResponse response,
		...
		try {
	       ...
	       if (  (ses != null && ses.getAttribute("usuarioOwncloud") != null && ses.getAttribute("senhaOwncloud") != null)
	                                  || reqURI.indexOf("/site/owncloud/body/") >= 0 && reqURI.contains("javax.faces.resource") )
	              chain.doFilter(request, response);
	       else   
	       	//Usuario não tem sessao aberta ainda, portanto o servidor redireciona ele para a pagina de login.
	              res.sendRedirect(req.getContextPath() + "/site/owncloud/login.xhtml");  
	 }
	catch(Throwable t) {
